...
bool WorldLootItem::IsContainer() {
	if (!this->cachedIsContainer.has_value()) {
		this->cachedIsContainer = Memory::ReadValue<uint64_t>(Global::pMemoryInterface, this->address + 0x18) == 0;
	}
...
Vector3 WorldLootItem::GetPosition() {
	if (!this->cachedTransformAddress) {
		this->cachedTransformAddress = Memory::ReadValue<uint64_t>(Global::pMemoryInterface, this->address + 0x10);
	}
...
std::optional<std::wstring> WorldLootItem::GetId() {
	if (!this->cachedId.has_value()) {
		std::wstring readString = Memory::ReadString(Global::pMemoryInterface, Memory::ReadValue<uint64_t>(Global::pMemoryInterface, this->address + 0x68));
